...
public class CreateAccount extends JFrame {
    ...
    public CreateAccount() {
        ...
        // Reduced gap
        mainPanel.setLayout(new GridLayout(1, 2, (int)(screenWidth * 0.015), 0));
        // Reduced margins
    ...
    private void createFormContent(JPanel formPanel) {
        ...
        int baseFontSize = Math.max(20, screenSize.width / 120);
        ...
        titleLabel.setFont(new Font("Segoe UI", Font.BOLD, Math.max(20, (int)(baseFontSize * 1.8))));
        titleLabel.setHorizontalAlignment(SwingConstants.CENTER);
        // Reduced margin
        titleLabel.setBorder(BorderFactory.createEmptyBorder(0, 0, (int)(screenSize.height * 0.01), 0));
        ...
        int verticalSpacing = Math.max(3, screenSize.height / 200);
        // Reduced insets
        gbc.insets = new Insets(verticalSpacing, 6, verticalSpacing/2, 6);
        gbc.anchor = GridBagConstraints.WEST;
        gbc.fill = GridBagConstraints.HORIZONTAL;
        gbc.weightx = 1.0;

        // Create responsive text fields with optimized size
        // Further reduced field size
        int fieldColumns = Math.max(15, screenSize.width / 100);
    ...
    private void addEnhancedField(JPanel panel, String labelText, Component component, int yPos) {
        ...
        int verticalSpacing = Math.max(12, screenSize.height / 250);
        // Further reduced spacing
        int horizontalSpacing = Math.max(12, screenSize.width / 300);
        ...
        gbc.insets = new Insets(verticalSpacing, horizontalSpacing, 1, horizontalSpacing);
        gbc.anchor = GridBagConstraints.WEST;

        JLabel label = new JLabel(labelText);
        // Smaller label font
        int labelFontSize = Math.max(20, screenSize.width / 160);
        ...
        gbc.insets = new Insets(0, horizontalSpacing, verticalSpacing, horizontalSpacing);
    ...
    private void createInfoContent(JPanel rightPanel) {
        ...
        String infoText =
                // Text content for the information section
                """
    ...
    // Gathers form data and saves to DB on a background thread.
    private void handleSubmit() {
        ...
        SwingWorker<Boolean, Void> worker = new SwingWorker<Boolean, Void>() {
            ...
            protected Boolean doInBackground() throws Exception {
                // Use the correct table name from the current src/DBConnector logic
                String sql = "INSERT INTO user_accounts (full_name, username, password, contact_number, email) VALUES (?, ?, ?, ?, ?)";
            ...
            protected void done() {
                try {
                    if (get()) {
                        JOptionPane.showMessageDialog(CreateAccount.this, "Account created successfully! You can now log in.");
                        CreateAccount.this.dispose();
                        SwingUtilities.invokeLater(() -> new PawTrackLogin().setVisible(true));
                    } else {
                        JOptionPane.showMessageDialog(CreateAccount.this,
                                "Error saving account: " + errorMessage,
    ...
    // @param resourcePath Path to the image file (relative to resources folder)
    ...
    private JLabel createImageLabel(String resourcePath, int width, int height) {
        ...
        try {
            ...
            if (resourceStream != null) {
                originalImage = ImageIO.read(resourceStream);
            }
            ...
            if (originalImage != null) {
                ...
                System.err.println("Could not load image resource: /" + resourcePath);
            }
